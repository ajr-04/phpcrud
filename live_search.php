...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['search'])) {
        ...
        try {
            ...
            if ($connection) {
                // SQL query with JOIN
                $query = $connection->prepare("SELECT users.user_id, users.user_fn, users.user_ln, users.user_birth, users.user_sex, users.user_name, users.user_profile_picture, CONCAT(user_address.user_city,', ', user_address.user_province) AS users_address FROM users INNER JOIN user_address ON users.user_id = user_address.user_id WHERE users.user_name LIKE ? OR users.user_id LIKE ?");
                ...
                foreach ($users as $user) {
                    ...
                    $html .= '<td>' . $user['user_fn'] . '</td>';
                    $html .= '<td>' . $user['user_ln'] . '</td>';
                    $html .= '<td>' . $user['user_birth'] . '</td>';
                    $html .= '<td>' . $user['user_sex'] . '</td>';
                    $html .= '<td>' . $user['user_name'] . '</td>';
                    $html .= '<td>' . $user['users_address'] . '</td>';
